...
const ListProducts = async (req, resp) => {
  try {
    ...
    let result = await (
      ...
      return cart.Customer._id === CustomerID
    })
    ...
    let Message = new MessageResponse(
      'There is an error listing the product from the shopping cart: ' + error,
      false,
      null,
    )
...
const AddProduct = async (req, resp) => {
  try {
    ...
    let result = await (
      ...
      return cart.Customer._id === Customer._id
    })
    ...
    if (result.length > 0) {
      ...
      if (hasAllElems.length > 0) {
        let Message = new MessageResponse('the product item is already registered', false, null)
        return resp.status(200).json(Message.GetMessage())
      }
    } else {
    ...
    let Message = new MessageResponse('The product has been added in the shopping cart.', true, null)
  ...
  try {
    ...
    if (filtered) {
      ...
      if (hasAllElems.length > 0) {
        ...
        //send the result
        let Message = new MessageResponse('The product has been removed from shopping cart.', true, null)
        return resp.status(200).json(Message.GetMessage())
      } else {
        //send the message
        let Message = new MessageResponse('There are not products to remove', false, null)
        return resp.status(200).json(Message.GetMessage())
      }
    } else {
      let Message = new MessageResponse('There are not customers to remove', false, null)
      return resp.status(200).json(Message.GetMessage())
    }
  } catch (error) {
    let Message = new MessageResponse(
